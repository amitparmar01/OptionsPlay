        ...
        this.loadData = function () {
            if (self.isValidMarketTime(new Date().valueOf()) || self.priceData.length <=0 ) {
                dataContext.stockQuotePerMin.get(ko.unwrap(self.stockCode)).done(function (datum) {
                    ...
                    self.closePriceData = [];
                    self.closePriceData.push([self.startTimeOfMorning, data.previousClose]);
                    self.closePriceData.push([self.adjustXAxisValue(self.closeTimeOfAfternoon), data.previousClose]);
